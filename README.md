...
- `GET /api/v1/admin/audit-log/export?limit=1000`
...
The committed `apps\mobile\eas.json` contains only credential-free build and submit profile metadata for development, private-beta, and production. Static mobile QA enforces that every profile targets `https://freemail.kuzuryu.ai` and that no signing or store credentials are committed.
...
Push notification dispatch has a deterministic development provider for `contract-only` and `development` registrations. The mobile client includes a one-tap development registration helper that creates a stable FreeMail device ID and development token in SecureStore for VPN beta delivery tests. Credential-backed `fcm` and `apns` adapters are available when the operator configures provider secrets through environment variables; otherwise those notifications are recorded as `pending_provider`.
...
The webmail preview can load live mailbox folders and message headers from the API. Start `admin-api`, `mail-core`, and `web`, open `http://127.0.0.1:18091`, enter a mailbox address/password, and keep the API field pointed at `http://127.0.0.1:18090`. The browser client exchanges the mailbox password for a bearer session at `POST /api/v1/mailbox/session`, stores only the bearer token in tab-scoped `sessionStorage`, clears any legacy persistent browser session entries, and revokes the current session with `DELETE /api/v1/mailbox/session`, one active mailbox session with `DELETE /api/v1/mailbox/sessions/{sessionId}`, or all active sessions with `DELETE /api/v1/mailbox/sessions`. The API stores mailbox passwords only as encrypted session material using `FREEMAIL_SESSION_SECRET`. FastAPI responses and the Caddy-served web shell set a baseline `Content-Security-Policy`, `Referrer-Policy`, `Permissions-Policy`, `X-Content-Type-Options`, `X-Frame-Options`, and `Cross-Origin-Opener-Policy`; static QA fails if the web container drops the Caddy security-header config. For a different local web origin, set `FREEMAIL_WEB_CORS_ORIGINS`.
...
.\.venv\Scripts\python.exe scripts\qa_mailbox_preferences_api.py --email admin@example.com --secrets-json secrets\mail-core-users.json
.\.venv\Scripts\python.exe scripts\qa_mailbox_import_api.py --email admin@example.com --secrets-json secrets\mail-core-users.json
```

`FREEMAIL_SESSION_SECRET` must be set for browser mailbox sessions. Use a long random value and keep it out of source control. Existing per-request mailbox credential QA scripts remain available for API smoke coverage. The preferences smoke restores the mailbox's prior display name and signature after verification.
...
POST /api/v1/mailbox/folder/empty
```

Create accepts `{ "folder": "Clients" }`, rename accepts `{ "folder": "Clients", "targetFolder": "Customers" }`, delete accepts `{ "folder": "Customers" }`, and empty accepts `{ "folder": "Deleted Items" }`. Empty marks every message in the selected folder as deleted and expunges the folder through IMAP; `INBOX`, `Sent Items`, `Drafts`, and `Archive` are protected from full-folder emptying. The webmail sidebar exposes create, rename-current-folder, empty-current-folder, and delete-current-folder controls while protecting core folders such as `INBOX`.
...
The current webmail preview supports reply and forward as live compose-prefill workflows from the selected message body. Messages selected from `Drafts` can be reopened into compose through Edit draft; attachment metadata is shown in the reader, and files should be reattached before saving or sending the reopened draft. Folder snapshots and search results accept `limit` plus `offset` and return `nextOffset`/`hasMore`; web and mobile clients expose Load more controls over that API contract. Message summaries and details also expose derived `threadId`, `threadSubject`, and `inReplyTo` metadata from standard message headers so clients can show conversation hints without requiring a separate thread store. Header inspection is available through `GET /api/v1/mailbox/message/headers?folder=...&message_id=...`, returning common identity fields, `Authentication-Results`, `List-Unsubscribe`, received-hop count, and full header name/value pairs without downloading the body. The thread lookup API loads messages in the same derived conversation within a folder:

```text
GET /api/v1/mailbox/thread?folder=INBOX&thread_id=thread-...
```

Star and Unstar update the IMAP `\Flagged` flag through the mailbox API and display starred messages with a leading marker in message lists. The bulk action API and clients can mark selected messages read/unread, star/unstar them, archive them, or move them to spam/trash in one request. Mailbox preferences are persisted in metadata with `GET`/`PUT /api/v1/mailbox/preferences`; web and mobile clients use the stored signature as the default compose signature. Sending compose drafts uses the same mailbox send API and server-side Sent Items persistence, while Save draft uses the draft API. Single-message EML export is available through `GET /api/v1/mailbox/message/source?folder=...&message_id=...`, and EML import is available through `POST /api/v1/mailbox/message/import` with `{ "folder": "...", "filename": "message.eml", "contentBase64": "..." }`. Web and mobile clients expose download/share/import controls for authenticated mailbox message portability. The Archive action uses IMAP copy/delete semantics, creates the `Archive` folder if it is missing, and refreshes the current folder after success:
