...
def main() -> int:
    parser = argparse.ArgumentParser(description="Probe the FreeMail mailbox move API.")
    ...
    redacted = {
        "trashMoved": trash["moved"],
        "trashRemovedFromSource": trash["removedFromSource"],
        "spamMoved": spam["moved"],
        "spamRemovedFromSource": spam["removedFromSource"],
    }
    print(json.dumps(redacted, indent=2, sort_keys=True))
    return 0 if all(redacted.values()) else 1


def _move_generated_message(
    base_url: str,
    email: str,
    password: str,
    folder: str,
    ...
    *,
    ...
    subject = f"FreeMail move API smoke {target_folder} {uuid.uuid4()}"
    ...
    moved = _move_message(base_url, email, password, folder, str(message["messageId"]), target_folder)
    removed = _wait_until_missing(base_url, email, password, folder, subject, attempts=attempts, interval=interval)
    return {"moved": bool(moved.get("moved")), "removedFromSource": removed}
...
def _send_message(base_url: str, email: str, password: str, recipient: str, subject: str) -> None:
    payload = {
        ...
        "subject": subject,
        "body": "FreeMail generated move API smoke message.",
    }
    response = _json_request(base_url, "/api/v1/mailbox/send", email, password, method="POST", payload=payload)
    if not response.get("accepted"):
        raise RuntimeError("generated move smoke message was not accepted")
...
def _wait_until_missing(
    ...
    *,
    ...
    for _attempt in range(max(1, attempts)):
        if _find_message(base_url, email, password, folder, subject) is None:
            return True
        time.sleep(max(0.1, interval))
    return False
...
def _move_message(
    ...
    payload = {"folder": folder, "messageId": message_id, "targetFolder": target_folder}
    return _json_request(base_url, "/api/v1/mailbox/message/move", email, password, method="POST", payload=payload)
