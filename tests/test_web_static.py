...
def test_static_web_validation_flags_credential_storage():
    ...
    failures = _validate(
        ...
        "fetch('/api/v1/mailbox/sender-rules/apply'); deleteMailboxSenderRule(1); applyMailboxSenderRules(); "
        "renderMailboxSenderRules([]); saveSelectedSenderRule('block'); "
        "fetch('/api/v1/mailbox/recipient-rules'); loadMailboxRecipientRules(); saveMailboxRecipientRule({}); "
        "deleteMailboxRecipientRule(1); renderMailboxRecipientRules([]); recipient-rules-refresh; "
        "recipient-rule-form; Recipient rules; "
        "senderEmailFromText('Sender <sender@example.net>'); allow-sender-action; block-sender-action; "
        "sender-rules-refresh; sender-rules-apply; Apply blocks; "
        ...
        "window.sessionStorage.setItem('freemail.mailboxSession', '{}'); "
        "window.sessionStorage.getItem('freemail.mailboxSession'); "
        "window.sessionStorage.removeItem('freemail.mailboxSession'); "
