...
The committed `eas.json` contains only credential-free build and submit profile metadata. Its `private-beta` profile uses `internal` distribution, physical-device iOS builds, Android `app-bundle` output, and the VPN API target. EAS signing credentials, store credentials, provisioning profiles, and keystores must stay in private release infrastructure.
...
Signed iOS and Android build artifacts are validated through the root `scripts/mobile_release_gate.py` evidence gate. Keep the evidence JSON and signed artifacts outside Git; the gate accepts only credential-free metadata, native build identifiers, artifact hashes, and the VPN-only private beta boundary. The `ios.buildNumber` and `android.versionCode` values in `app.json` must match the signed-build and store-submission evidence for a release candidate. Use `--require-store-submission` after TestFlight and Play internal-testing submission evidence exists.
