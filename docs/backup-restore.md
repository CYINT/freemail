...
The metadata backup tools cover domains, users, encrypted administrator MFA secrets, mailboxes, aliases, DKIM keys, and audit logs. They intentionally exclude admin bearer sessions, browser mailbox sessions, outbound rate-limit counters, push-device registrations, encrypted push-provider tokens, and push-notification delivery records.
