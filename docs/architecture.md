...
The admin API owns FreeMail metadata for domains, users, user-password rotation, administrator MFA, administrator bearer sessions, mailboxes, aliases, and audit logs. The current implementation uses SQLite through explicit repository functions so the early product can keep a small dependency surface while still proving persistence and API contracts.
