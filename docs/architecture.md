...
The admin API owns FreeMail metadata for domains, users, invitation-link signup, user-password rotation, administrator MFA, administrator bearer sessions, mailboxes, aliases, and audit logs. The current implementation uses SQLite through explicit repository functions so the early product can keep a small dependency surface while still proving persistence and API contracts.
...
Admin password login verifies active administrator users against stored Argon2id password hashes, enforces TOTP when authenticator-app MFA is enabled, creates a hashed bearer session, and stores no admin password material in runtime session tables. Administrators can inspect their own active bearer sessions, identify the current session, revoke a selected session, or revoke all of their admin sessions; static operator-token access remains outside this bearer-session table. Admin TOTP setup secrets are returned once for enrollment, stored encrypted with `FREEMAIL_SESSION_SECRET`, and audited when setup, enabled, or disabled. Admin invitation links store only SHA-256 token hashes, return the raw token once, prefill public signup metadata, and are single-use. Admin password rotation replaces the stored Argon2id hash, revokes existing admin sessions for the target user, and records an audit event. Suspending a user deletes that user's admin sessions and mailbox sessions for their mailboxes; suspending a mailbox or domain deletes affected mailbox sessions. Session resolution still rechecks user, mailbox, and domain status for defense in depth.
...
- `user_invitations`: expiring, single-use invitation-link records with hashed tokens
