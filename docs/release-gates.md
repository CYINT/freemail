...
The packet status command is read-only. By default it discovers `.freemail-qa\release\release-evidence-manifest.json` when present and checks the committed `docs\release-notes\v0.1.0-private-beta.md`; pass `--manifest` when using a different manifest path. If the manifest leaves mobile or private-beta evidence blank, packet status falls back to conventional external evidence paths such as `.freemail-qa\mobile-release-evidence.freemail.kuzuryu.ai.json`, `.freemail-qa\mobile-release-evidence.json`, `.freemail-qa\private-beta-gate.freemail.kuzuryu.ai.json`, and `.freemail-qa\private-beta-gate.json`. It reports missing, empty, and invalid artifacts, runs the local restore-drill, mobile, private-beta, and release-notes evidence checks, records SHA-256 checksums for present artifacts, and includes credential-free `nextActions` when mobile signed-build/device-validation/store-submission evidence or private-beta acceptance/evidence is incomplete. Passing packet status does not replace the full release gate because it intentionally excludes Git, GitHub Actions, Docker Compose, runtime health, runtime security-header, deployment-boundary, product-readiness, metadata-readiness, mail-core-readiness, and hosted mobile app-link association checks.
...
This mobile status command is read-only and reports missing or failing signed-build and store-submission checks without running native build tools or contacting store APIs. Failed mobile checks include a `failedRequirements` list for missing or stale app metadata, signed-build fields, real-device validation fields, and store-submission fields, plus `nextActions` with placeholder collector commands for the missing evidence. By default, mobile evidence helpers use `.freemail-qa\mobile-release-evidence.freemail.kuzuryu.ai.json`, falling back to an existing `.freemail-qa\mobile-release-evidence.json` only for backward compatibility. The top-level packet status command expects `--require-mobile-store-submission` whenever mobile evidence is present; `--allow-pre-store-mobile-packet` is reserved for explicit pre-store dry runs and must not be used as final release evidence.
...
- mobile signed-build, real-device private-beta validation, and store-submission evidence passes `scripts/mobile_release_gate.py` with credential-free proof for both iOS and Android; real-device validation must include VPN DNS resolution, auth login, inbox sync, message read, compose/send, HTTPS invite-link app opening, and offline cache checks; use `scripts/collect_mobile_build_evidence.py`, `scripts/collect_mobile_device_validation.py`, and `scripts/collect_mobile_store_submission.py` for those records
