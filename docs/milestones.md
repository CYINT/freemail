...
Current progress: Expo/React Native scaffold exists under `apps/mobile` with typed mailbox API client, SecureStore-backed bearer-session persistence, default VPN hostname `https://freemail.kuzuryu.ai`, inbox/read/compose/reply/forward/sign-out UI, folder navigation and management, folder-scoped search, contacts, attachment metadata/availability checks, bounded document-picker compose attachments, secure offline metadata caching for the last loaded mailbox views, bearer-authenticated push-device registration/list/revoke contracts, AGPL package metadata, Expo config validation, native release/signing documentation, and static QA in `scripts/qa_mobile_static.py`. Remaining work is native iOS/Android build drill evidence, provider-specific push delivery, richer attachment download/save UX, and app-store release execution.
